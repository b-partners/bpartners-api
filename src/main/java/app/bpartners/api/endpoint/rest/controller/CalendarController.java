...
public class CalendarController {
  ...
  public List<CalendarEvent> getCalendarEvents(
      ...
      @RequestParam(name = "provider", required = false) CalendarProvider provider,
  ...
  public List<CalendarEvent> crupdateEvents(
      @PathVariable(name = "idUser") String idUser,
      @PathVariable(name = "calendarId") String idCalendar,
      @RequestBody(required = false) List<CreateCalendarEvent> events) {
    List<app.bpartners.api.model.CalendarEvent> domainEvents =
        events.stream().map(mapper::toDomain).collect(Collectors.toList());
  ...
  public TokenValidity handleAuth(
      @PathVariable(name = "id") String idUser, @RequestBody CalendarAuth auth) {
