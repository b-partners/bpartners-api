...
public class DbTransactionRepository implements TransactionRepository {
  ...
  public Transaction save(JustifyTransaction toSave) {
    ...
    return mapper.toDomain(
        savedTransaction, categoryRepository.findByIdTransaction(entity.getId()));
  }

  @Override
  public List<Transaction> saveAll(List<Transaction> transactions) {
    List<HTransaction> entities = transactions.stream().map(mapper::toEntity).toList();
    ;
    return entities.stream()
        .map(
            entity ->
                mapper.toDomain(entity, categoryRepository.findByIdTransaction(entity.getId())))
        .collect(Collectors.toList());
  }
