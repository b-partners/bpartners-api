...
@AllArgsConstructor
@Slf4j
public class FacadeCalendarEventRepository implements CalendarEventRepository {
  ...
  // TODO: improve using correctly facade and connectors design pattern, ie, use Google and Local
  // Repository
  @Override
  public List<CalendarEvent> saveAll(String idUser, String idCalendar, List<CalendarEvent> toSave) {
    ...
    List<EventConnector> googleEventsToSave =
        toSave.stream()
            .map(
                domain -> {
                  String eteId = null;
    ...
    List<HCalendarEvent> eventEntities =
        savedGoogleEventConnectors.isEmpty()
            ? toSave.stream()
    ...
    return savedEntities.stream().map(eventMapper::toDomain).collect(Collectors.toList());
  }
  ...
  // TODO: improve this
  private List<EventConnector> getSavedGoogleEvents(
      String idUser, Calendar calendar, List<EventConnector> googleEvents) {
    try {
      googleEvents.forEach(
          event -> {
            var savedEvents =
                calendarApi.crupdateEvents(
                    idUser, calendar.getEteId(), List.of(event.getGoogleEvent()));
