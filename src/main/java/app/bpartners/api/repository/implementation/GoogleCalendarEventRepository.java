...
@AllArgsConstructor
@Slf4j
public class GoogleCalendarEventRepository implements CalendarEventRepository {
  ...
  public List<CalendarEvent> findByIdUserAndIdCalendar(
    ...
    List<CalendarEvent> newGoogleEvents =
        googleConvertedEvents.stream().filter(CalendarEvent::isNewEvent).toList();
    if (!newGoogleEvents.isEmpty()) {
      jpaRepository.saveAll(
          newGoogleEvents.stream()
    ...
    return crupdatedEvents.stream()
        .map(
            googleEvent -> {
              ...
              List<HCalendarEvent> optionalEvent =
                  jpaRepository.findAllByEteId(googleEvent.getId());
              if (!optionalEvent.isEmpty()) {
                List<HCalendarEvent> sortedByUpdatedAtDesc =
                    optionalEvent.stream()
                ...
                if (sortedByUpdatedAtDesc.remove(eventEntity)) {
                  jpaRepository.deleteAllById(
                      sortedByUpdatedAtDesc.stream()
