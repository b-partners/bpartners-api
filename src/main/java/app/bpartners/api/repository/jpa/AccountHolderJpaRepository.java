...
public interface AccountHolderJpaRepository extends JpaRepository<HAccountHolder, String> {
  ...
  @Query(value =
      "select new HAccountHolder (ac.id, ac.accountId, ac.socialCapital, ac.vatNumber,"
          ...
          + "ac.businessActivityDescription, ac.feedbackLink, ac.address, ac.city, ac.country,"
          + " ac.postalCode, ac.longitude, ac.latitude, ac.townCode, ac.prospectingPerimeter)"
          + " from HAccountHolder ac group by ac.accountId, ac.id, ac.socialCapital,"
          + " ac.vatNumber, ac.mobilePhoneNumber, ac.email, ac.initialCashflow, ac.feedbackLink")
