...
public interface TransactionCategoryTemplateJpaRepository
  ...
  @Query(
      value =
          "select template.*, case when count is null then 0 else count end from"
              + " transaction_category_template template         left join (select"
              + " id_category_tmpl, count(id_category_tmpl) as count                    from"
              + " (select template.id as id,       tc.id_transaction,       tc.id_account,      "
              + " template.id as id_category_tmpl,       tc.comment  as comment from"
              + " transaction_category_template template         join transaction_category tc on   "
              + " (template.id = tc.id_transaction_category_tmpl) where (tc.id_account = ?1    or"
              + " tc.id_account is null) and tc.created_datetime between ?2 and ?3) as"
              + " template_view                    group by (template_view.id_category_tmpl,"
              + " comment)) as template_count                   on template.id ="
              + " template_count.id_category_tmpl",
      nativeQuery = true)
