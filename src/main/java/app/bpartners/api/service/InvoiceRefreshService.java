        ...
        user -> {
          List<Invoice> invoices =
              invoiceService.getInvoices(
                  ...
                  null,
                  List.of());
          List<Invoice> unpaidRegInvoices =
              invoices.stream()
                  .filter(
                      invoice -> {
                        ...
                        List<PaymentRequest> payments =
                            paymentType == PaymentTypeEnum.CASH
                                ? paymentRequestRepository.findAllByReference(
                                    invoice.getRealReference())
                                : List.of();
                        return updatedAtDeploy
                            && (paymentType == PaymentTypeEnum.IN_INSTALMENT
                                ...
                                || paymentType == PaymentTypeEnum.CASH
                                    ...
                                    && payments.stream()
                                        .allMatch(
                                            paymentRequest ->
                                                paymentRequest.getStatus() == UNPAID));
                      })
                  .toList();

          unpaidRegInvoices.forEach(
              invoice -> {
                try {
                  ...
                  log.error(
                      "An error occurred during processing for invoice {}: {}",
                      invoice.describe(),
                      e.getMessage());
                }
              });
