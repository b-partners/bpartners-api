...
public class S3Service {
  ...
  public String presignURL(
    ...
    return customBucketComponent.presign(key, Duration.ofSeconds(expirationInSeconds)).toString();
  }

  @SneakyThrows
  public FileHash uploadFile(FileType fileType, String fileId, String idUser, File fileToUpload) {
    String key = bucketKeyRetriever.apply(fileType, fileId, idUser);
    return customBucketComponent.upload(fileToUpload, key, true);
  }

  public File downloadFile(FileType fileType, String fileId, String idUser) {
    String key = bucketKeyRetriever.apply(fileType, fileId, idUser);
    return customBucketComponent.download(key, true);
  }

  public File downloadLandingFile(String key) {
    return customBucketComponent.download(key, false);
  }

  public String uploadLandingFile(File file, String key) {
    customBucketComponent.upload(file, key, false);
    return customBucketComponent.presignLanding(key, Duration.ofSeconds(60), false).toString();
  }
}
