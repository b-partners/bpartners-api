...
public class UserAnalyseApiKeyService {
  private static final String GEOJOBS_BASE_URL = "api.birdia.fr"; // TODO: get from environment variables
  ...
  private static final List<DetectableObjectModel> DEFAULT_ALLOWED_MODELS = List.of(DetectableObjectModel.ofName("BP_TOITURE"));

  private static final List<AuthorizedZone> DEFAULT_AUTHORIZED_ZONES = List.of(); // deprecated
  private static final List<DetectableObjectType> DEFAULT_DETECTABLE_OBJECT_TYPES = List.of(); // deprecated

  private final RestTemplate restTemplate;

  public UserAnalysisApiKey getAnalysisApiKey(User user) {
    UriComponentsBuilder uriBuilder = UriComponentsBuilder.fromHttpUrl(GEOJOBS_BASE_URL + "/api/keys");
    ...
    CreatedAnalysisApiKey createdAnalysisApiKey = restTemplate.postForObject(
        uriBuilder.toUriString(),
        request,
        CreatedAnalysisApiKey.class
    );

    return new UserAnalysisApiKey().toBuilder()
        .userId(user.getId())
  ...
  enum ConsumerType { INSURANCE, COMMUNITY, ADMIN }
