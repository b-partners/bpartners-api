...
$$
begin
            if not exists(select from pg_type where typname = 'transaction_status') then
...
alter table transaction add column currency varchar;
alter table transaction add column label varchar;
alter table transaction add column reference varchar;
alter table transaction add column side varchar;
alter table transaction add column status transaction_status;
alter table transaction add column payment_date_time date;
