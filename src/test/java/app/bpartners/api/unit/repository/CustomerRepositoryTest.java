...
class CustomerRepositoryTest {
    ...
    @InjectMocks
    private CustomerRepositoryImpl customerRepositoryImpl; // Suppose que c'est l'implémentation de CustomerRepository
    ...
    void findByIdAccountHolder_returnsCustomerList() {
        when(customerRepository.findByIdAccountHolder("account1"))
                .thenReturn(List.of(customer));
    ...
    void saveAll_returnsSavedCustomers() {
        when(customerRepository.saveAll(List.of(customer)))
                .thenReturn(List.of(customer));
    ...
    void findByIdUserAndCriteria_returnsFilteredCustomerList() {
        when(customerRepository.findByIdUserAndCriteria(
                "user1", "John", "Doe", "john.doe@example.com", "123456789",
                "Anytown", "Countryland", List.of(), "prospect1",
                CustomerStatus.ENABLED, 0, 10))
                .thenReturn(List.of(customer));

        List<Customer> customers = customerRepository.findByIdUserAndCriteria(
                "user1", "John", "Doe", "john.doe@example.com", "123456789",
                "Anytown", "Countryland", List.of(), "prospect1",
                CustomerStatus.ENABLED, 0, 10);
        assertNotNull(customers);
    ...
    void updateCustomersStatuses_returnsUpdatedCustomers() {
        ...
        List<Customer> updatedCustomers = customerRepository.updateCustomersStatuses(
                List.of(new UpdateCustomerStatus()));
    ...
    void findWhereLatitudeOrLongitudeIsNull_returnsCustomers() {
        when(customerRepository.findWhereLatitudeOrLongitudeIsNull())
                .thenReturn(List.of(customer));
