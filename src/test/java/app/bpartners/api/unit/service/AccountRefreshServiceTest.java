...
import app.bpartners.api.model.BankConnection.BankConnectionStatus;
...
class AccountRefreshServiceTest {
    ...
    void refreshDisconnectedUsersShouldHandleExceptionDuringAuthentication() {
        ...
        users.add(user);
        when(userServiceMock.findAll()).thenReturn(users);
        when(bridgeApiMock.authenticateUser(any(CreateBridgeUser.class))).thenThrow(new RuntimeException("Authentication failed"));
