...
class PaymentScheduleServiceTest {
  ...
  PaymentScheduleService subject =
      new PaymentScheduleService(infoRepositoryMock, jpaRepositoryMock);
  ...
  void payment_message() {
    var paymentRequest =
        HPaymentRequest.builder().id("paymentId").sessionId("sessionId").status(PAID).build();
